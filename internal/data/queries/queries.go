...
func AddReaction(PID, UID int, typee string)  error {
	statement, err := database.Db.Prepare(`INSERT INTO likes (post_id, user_id, reaction) VALUES (?, ?, ?) ON CONFLICT(user_id, post_id) DO UPDATE SET reaction = excluded.reaction`)
	if err != nil {
		return err
	}
	defer statement.Close()
	_, err = statement.Exec(PID, UID, typee)
	return err
}


func RemoveReaction(PID, UID int) error {
